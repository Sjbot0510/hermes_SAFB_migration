...
The scalar field $\psi(\boldsymbol{r})$ is expanded as a Fourier series over symmetry-adapted modes:

\begin{equation}
    \psi(\boldsymbol{r}) = \sum_{n} A_n \exp(i \boldsymbol{q}_n \cdot \boldsymbol{r})
\end{equation}

where:
\begin{itemize}
    \item $\boldsymbol{q}_n$ are the reciprocal lattice vectors (from stars)
    \item $A_n$ are complex amplitudes constrained by symmetry
\end{itemize}

The Fourier coefficient for a reciprocal vector $\boldsymbol{q}$ is:
\begin{equation}
    c_{\boldsymbol{q}} = \frac{1}{\Omega} \int_{\Omega} \psi(\boldsymbol{r}) \exp(-i \boldsymbol{q} \cdot \boldsymbol{r}) \, d\boldsymbol{r}
\end{equation}
...
Symmetry imposes constraints on the Fourier coefficients. For a symmetry operation $(\Rmat, \boldsymbol{t})$ and a reciprocal vector $\boldsymbol{q}$, the coefficients satisfy:

\begin{equation}
    c_{\Rmat \boldsymbol{q}} = \exp(2\pi i \, \boldsymbol{t} \cdot \boldsymbol{q}) \, c_{\boldsymbol{q}}
\end{equation}
...
The phase factor for a symmetry operation $(\Rmat, \boldsymbol{t})$ acting on vector $\boldsymbol{q} = (h, k, l)$ is:

\begin{equation}
    \phi = 2\pi \, \boldsymbol{t} \cdot \boldsymbol{q} = 2\pi \sum_{j} t_j q_j
\end{equation}
...
For centered rectangular lattices, the centering transformation $\boldsymbol{r}' = \boldsymbol{r} + (a/2, b/2)$ leads to:
...
\begin{equation}
    \psi(\boldsymbol{r}) = c_{0,0} + \sum_{(k_1,k_2) \in D_6} c_{k_1,k_2} \sum_{j=0}^{5} \exp(i \boldsymbol{q}_j \cdot \boldsymbol{r})
\end{equation}

where the six vectors $\boldsymbol{q}_j$ are generated by 6-fold rotation from $(k_1,k_2)$. Domain D6: $0 < k_1$, $0 \leq k_2 < k_1$.

\subsection{p4 (Square, 4-fold symmetry)}

\begin{equation}
    \psi(\boldsymbol{r}) = c_{0,0} + \sum_{(k_1,k_2) \in D_4} c_{k_1,k_2} \sum_{j=0}^{3} \exp(i \boldsymbol{q}_j \cdot \boldsymbol{r})
\end{equation}
...
\begin{equation}
    \psi(\boldsymbol{r}) = c_{0,0} + \sum_{(k_1,k_2) \in D_2} c_{k_1,k_2} \left[ \exp(i \boldsymbol{q}_{k_1,k_2} \cdot \boldsymbol{r}) + \exp(i \boldsymbol{q}_{-k_1,-k_2} \cdot \boldsymbol{r}) \right]
\end{equation}
...
\begin{equation}
    \psi(\boldsymbol{r}) = c_{0,0} + \sum_{k_1} c_{k_1,0} \exp(i k_1 \boldsymbol{b}_1 \cdot \boldsymbol{r}) + 2 \sum_{k_1, k_2 > 0} \text{Re}\left[c_{k_1,k_2} \exp(i(k_1 \boldsymbol{b}_1 + k_2 \boldsymbol{b}_2) \cdot \boldsymbol{r})\right]
\end{equation}
...
For wallpaper groups that contain inversion symmetry (2-fold rotation at the origin), all Fourier coefficients $c_{\boldsymbol{q}}$ must be \textbf{real}:

\begin{equation}
    c_{\boldsymbol{q}} \in \mathbb{R} \quad \text{for centrosymmetric groups}
\end{equation}
...
For groups without inversion symmetry, the reality condition $c_{-\boldsymbol{q}} = c_{\boldsymbol{q}}^*$ relates coefficients within the minimal domain:

\begin{equation}
    c_{-\boldsymbol{q}} = c_{\boldsymbol{q}}^*
\end{equation}
...
\begin{equation}
    \psi_{\text{norm}}(\boldsymbol{r}) = \frac{1}{2} \left[ 1 + \tanh\left( \frac{\psi(\boldsymbol{r}) - \langle\psi\rangle}{\sigma \cdot \text{std}(\psi)} \right) \right]
\end{equation}
...
\begin{equation}
    \langle|\psi|^2\rangle = \frac{1}{V} \int_V |\psi(\boldsymbol{r})|^2 \, d\boldsymbol{r}
\end{equation}
